...
4. **新刊通知** : ライブラリ表示時 (古い順に最大 8 件) / タイトルを開いた時 (そのシリーズ) に `NewReleaseChecker` が検索し、未登録かつ最新巻より新しい巻があれば追加 + ローカル通知をスケジュール (6 時間の再チェック間隔つき)。詳細は 4-4 を参照。
...
- **巻数表記のゆらぎ対応**: 全角数字「（４）」(`normalizeWidth`)、無番号の第1巻 (タイトル＝シリーズ名)、**上巻/中巻/下巻・前編/後編の分冊** (`BookMetadataParser.volumeOrdinal` で上/前=1, 中=2, 下/後=最後 のように順序を連番化) に対応。数字の巻が 1 つも無く 2 冊以上が上/中/下 等を持つ場合に連番割り当てを行う。
...
登録数が多いと全件チェックは時間がかかる (楽天は約 1 req/秒) ため、画面表示をブロックしない設計にしている。

- **ライブラリ表示時** (`MangaListViewModel.autoCheckNewReleases`): DB から即座に一覧を表示したうえで、**未完シリーズのうち最後にチェックした時刻が最も古い順に最大 8 件だけ**をバックグラウンドでチェックし、各件完了ごとに一覧を更新する。登録数が多くても表示・操作はブロックしない。
- **タイトルを開いたとき** (`MangaDetailViewModel.checkNewReleasesOnAppear`): その 1 シリーズだけをチェックして詳細を更新する。ライブラリで自動チェックされなかった作品も、開けば最新化される。
- **Pull to Refresh** (`refreshAllNewReleases`): 未完シリーズ全件を強制チェックする (ユーザー操作なので時間がかかってよい)。
- **再チェック間隔**: 各シリーズは `NewReleaseChecker` が `UserDefaults` に最終チェック時刻を保存し、**6 時間以内は再チェックをスキップ** (`force` 指定時を除く)。短時間に何度も画面を開いても API を叩きすぎない。
- 起動時の全件チェックは廃止 (重いため)。ライブラリは起動時の最初のタブなので、表示時の自動チェックで実質的にカバーされる。
